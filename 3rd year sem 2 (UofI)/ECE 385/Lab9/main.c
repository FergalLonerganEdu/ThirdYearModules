...
void KeyExpansion(uchar key[17], uint schedule[44])
{
	int i;
	uint temp;
	for(i = 0; i < 4; i++)
...
void AddRoundKey(uchar state[17], uint roundkey[4])
{
	int i;
	for(i = 0; i < 4 ; i++)
	{
...
void MixColumns(uchar state[17])
{
	int i;
	uchar a, b, c, d;
	for(i = 0; i < 4; i++)
...
void encryption(uchar state[17], uchar message[17], uint schedule[44])
{
	int i, k, m;
	uint temp[4];
	for(i = 0; i < 16; i++)
...
int main()
{
	...
	unsigned char newtext[17];
	unsigned char newkey[17];
